...
import { nextServer } from "@/lib/api/serverApi";
...
function ratingOf(camper: Camper): number {
  ...
  if (r.length > 0) {
    const sum = r.reduce((s, it) => s + (it.reviewer_rating ?? 0), 0);
    return sum / r.length;
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filtered = all.filter((c) => {
      ...
      if (q.vehicleType) {
        const form = (c.form ?? "").toString().trim();
        if (form !== q.vehicleType) return false;
      }
    ...
    if (q.sort === "price") {
      ...
      sorted.sort((a, b) => {
        ...
        return q.order === "asc" ? ra - rb : rb - ra;
      });
    }
